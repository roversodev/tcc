...
export function EventCalendar({
  ...
  const handleEventSave = (event: CalendarEvent) => {
    if (event.id) {
      ...
      toast(`Evento "${event.title}" atualizado`, {
        description: format(new Date(event.start), "dd 'de' MMMM, yyyy", { locale: ptBR }),
      })
    } else {
      onEventAdd?.({
        ...event,
  ...
  const handleEventDelete = (eventId: string) => {
    ...
    if (deletedEvent) {
      toast(`Evento "${deletedEvent.title}" deletado`, {
        description: format(new Date(deletedEvent.start), "dd 'de' MMMM, yyyy", { locale: ptBR }),
      })
    }
  }
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    ...
    toast(`Evento "${updatedEvent.title}" movido`, {
      description: format(new Date(updatedEvent.start), "dd 'de' MMMM, yyyy", { locale: ptBR }),
    })
  }
